...
|-----|--------|----------------------|
| **Curbcue** | All functionality available without sign-in | Open app → browse compare/crowd-meter without login. Optional sign-in under Settings for beta sync. |
| **ThrottleLink** | All functionality available without sign-in | Open app → accept safety notice → guest mode unlocks rides/map. Optional sign-in under Settings. |
| **CoachCore** | Some features restricted | Email `feedback@macksims.com` for test credentials, or use demo bypass if shown on login screen. |
| **Sermon Studio** | Some features restricted | Email `feedback@macksims.com` for test credentials. |
