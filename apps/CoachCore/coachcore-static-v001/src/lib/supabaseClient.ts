...
export type SupabaseConnectionState = "missing" | "checking" | "connected" | "error";

export async function checkSupabaseConnection(): Promise<{
  state: Exclude<SupabaseConnectionState, "checking" | "missing">;
  detail: string;
}> {
  ...
  return {
    state: "connected",
    detail: `Reachable at ${SUPABASE_URL} (auth endpoint OK — tables/schema wiring is next).`,
  };
}
