...
export function MockActionRunner({
  ...
  function handleRun() {
    setHasRun(true);
    if (actionLabel) {
      logCoachAction(actionLabel, successTitle);
    }
  }
  ...
  return (
    <div>
      ...
      >
        {hasRun ? "Saved on this device" : buttonLabel}
      </button>
      ...
      {hasRun ? (
        ...
        <p className="mt-4 text-xs leading-5 text-slate-500">
          Saves to this device and your coach activity log. Cloud sync runs when Supabase is connected.
        </p>
      )}
    </div>
  );
