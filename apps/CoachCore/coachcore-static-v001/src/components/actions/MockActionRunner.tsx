...
export function MockActionRunner({
  ...
  function handleRun() {
    setHasRun(true);
    if (actionLabel) {
      logCoachAction(actionLabel, `${successTitle} (local draft only)`);
    }
  }
    ...
    return (
      <div className="mt-6 rounded-3xl border border-white/10 bg-slate-950/40 p-5">
        ...
        <button
          type="button"
          disabled
          className="mt-4 w-full cursor-not-allowed rounded-2xl border border-white/10 px-5 py-3 font-black text-slate-500"
        >
  ...
  return (
    <div>
      ...
      >
        {hasRun ? "Local draft saved" : buttonLabel}
      </button>

      {hasRun ? (
        <div className="mt-5 rounded-3xl border border-sky-300/25 bg-sky-300/10 p-5">
          <p className="text-sm font-bold uppercase tracking-[0.25em] text-sky-200">Local draft only</p>
          ...
          <p className="mt-3 text-xs leading-5 text-sky-100/70">
            Example workflow preview. No athlete notification, database write, or partner API call was made.
          </p>
        ...
        <p className="mt-4 text-xs leading-5 text-slate-500">
          Explicit example mode. Saves a local draft on this device only.
        </p>
      )}
    </div>
  );
