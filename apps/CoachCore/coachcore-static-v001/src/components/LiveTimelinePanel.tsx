...
function buildTimeline(): TimelineItem[] {
  ...
  return [...mockItems, ...actionItems, ...checkInItems].sort((a, b) => b.sortKey - a.sortKey);
}
...
export function LiveTimelinePanel({ limit = 8 }: { limit?: number }) {
  ...
  useEffect(() => {
    ...
    const offLocal = onLocalDataChanged((scope) => {
      if (scope === "all" || scope === "actionLog" || scope === "checkIns" || scope === "assignments") {
        setTick((n) => n + 1);
      }
    });
    return () => {
  ...
  if (items.length === 0) {
    return (
      <p className="text-sm text-slate-400">No timeline events yet. Run a mock action to seed this device.</p>
    );
  }
