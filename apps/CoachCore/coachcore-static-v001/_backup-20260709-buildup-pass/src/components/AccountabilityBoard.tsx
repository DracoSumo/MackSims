...
export function AccountabilityBoard() {
  const [messageCompleteAthletes, setMessageCompleteAthletes] = useState<Set<string>>(new Set());

  useEffect(() => {
    const refresh = () => {
      const ids = new Set(
        listCompletions()
          .filter((row) => row.itemType === "message" && row.athleteId)
          .map((row) => row.athleteId as string),
      );
      setMessageCompleteAthletes(ids);
    };
    refresh();
    const off = onLocalDataChanged((scope) => {
      if (scope === "all" || scope === "assignments") refresh();
    });
    return () => off();
  }, []);
        ...
        {athletes.map((athlete) => (
          <Card key={athlete.id} title={athlete.name} subtitle={athlete.status}>
            ...
            <AthleteProgressBar
              ...
              messagesComplete={messageCompleteAthletes.has(athlete.id)}
            />
            <Link
              href={`/app/athletes/${athlete.id}`}
              className="mt-4 inline-block font-bold text-sky-300 hover:text-sky-200"
            >
              View profile →
            </Link>
          </Card>
