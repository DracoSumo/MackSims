...
export function FareComparisonPanel({ estimates, market }: FareComparisonPanelProps) {
    ...
    return (
      <section className="panel fare-panel">
        <div className="section-heading">
          ...
          <h2>No estimates for this trip</h2>
        </div>
        <p className="subtle-copy">
          Simulated fares are only available for supported pilot markets. Try Bermuda routes or check pickup and destination.
        </p>
      </section>
    );
  }
  ...
  return (
    <section className="panel fare-panel">
      <div className="section-heading">
        <p className="eyebrow">Fare comparison</p>
        <h2>Simulated trip options at a glance</h2>
        <p className="subtle-copy">Not live Uber/Lyft pricing — demo estimates for beta comparison only.</p>
      </div>
        ...
        {summaryItems.map(({ label, estimate }) => {
          ...
          return (
            <div className="comparison-cell" key={label}>
              ...
              <small>{estimate.etaMinutes} min ETA (simulated)</small>
            </div>
          );
        })}
      </div>
